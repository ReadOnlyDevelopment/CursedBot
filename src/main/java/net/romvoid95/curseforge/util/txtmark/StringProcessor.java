...
import net.romvoid95.curseforge.util.txtmark.enums.BlockType;
import net.romvoid95.curseforge.util.txtmark.enums.LineType;
...
public class StringProcessor {
     ...
     *            The input reader.
     */
    private StringProcessor(final Reader reader, final Configuration config)
    {
    	this();
        this.reader = reader;
        this.config = config;
    ...
    public void setInputToProcess(final String input) {
    	this.reader = new StringReader(input);
    }
    
    public StringProcessor addReplacer(String replace, String replaceWith) {
    	this.replacers.add(Replacer.replace(replace, replaceWith));
    	return this;
    }
    ...
    {
        final StringProcessor p = new StringProcessor(!(reader instanceof BufferedReader) ? new BufferedReader(reader) : reader,
                configuration);
        return p.process();
    }
    ...
    {
        final StringProcessor p = new StringProcessor(new BufferedReader(new InputStreamReader(input, configuration.encoding)),
                configuration);
        return p.process();
    }
    ...
    private String process() throws IOException
    {
        ...
        if(this.replacers.isEmpty()) {
        	...
        	String replacedOut = out.toString();
            for(Replacer repl : this.replacers) {
            	replacedOut = replacedOut.replace(repl.replace, repl.replaceWith);
            }
            return replacedOut;
		}
    }
    
    public String runProcessor() {
    	try {
			return this.process();
		} catch (IOException e) {
			e.printStackTrace();
    ...
    static class Replacer {
    	...
    	static Replacer replace(String replace, String replaceWith) {
    		return new Replacer(replace, replaceWith);
    	}
    	
		Replacer(String replace, String replaceWith) {
